...
MAX_LIQUIDITY_ALERTS_PER_CYCLE = 8

ALERT_COOLDOWN_SECONDS = 45 * 60
LIQUIDITY_COOLDOWN_SECONDS = 20 * 60
...
WATCH_VOLUME_PERCENT = 25
EARLY_VOLUME_PERCENT = 80
VIP_VOLUME_PERCENT = 110
ULTRA_VIP_VOLUME_PERCENT = 140

MAX_ENTRY_DISTANCE_PERCENT = 3.0
MAX_WATCH_DISTANCE_PERCENT = 6.0
ULTRA_MAX_DISTANCE_PERCENT = 2.0

FOMO_MAX_DISTANCE_PERCENT = 3.0
...
def send_photo(photo_path, caption="", chat_id=None):
    ...
    try:
        with open(photo_path, "rb") as photo:
            ...
            print("Telegram photo:", r.status_code, r.text[:120], flush=True)
    except Exception as e:
        print("send_photo error:", e, flush=True)
...
def can_alert(symbol, alert_type, cooldown=ALERT_COOLDOWN_SECONDS):
    alerts = load_alerts()
...
def get_klines(symbol, interval="1m", limit=120):
    ...
    try:
        ...
        print(f"get_klines error {symbol} {interval}:", e, flush=True)
        return []
...
def get_current_price(symbol):
    try:
        ...
        print("get_current_price error:", e, flush=True)
...
def liquidity_strength(a):
    if a["p30"] >= 180 or a["vol_diff30"] >= WHALE_VOLUME_DIFF_USDT * 3:
        return "🚨 سيولة ضخمة جدًا"
    if a["p30"] >= 100 or a["vol_diff30"] >= WHALE_VOLUME_DIFF_USDT:
        return "🔥 سيولة قوية"
    if a["p30"] >= 55:
        return "📊 سيولة جيدة"
    return "👀 مراقبة فوليوم"


def liquidity_score(a):
    volume_part = min(a["p30"], 200) * 0.5
    diff_part = min(a["vol_diff30"] / 10000, 80)
    price_part = max(0, min(a["price_change_10m"] * 8, 40))
    anti_dump = 20 if a["price_change_10m"] > -1.5 else 0

    return volume_part + diff_part + price_part + anti_dump
...
def analyze_liquidity(symbol):
    ...
    distance_1m = ((price - support_1m) / support_1m) * 100 if support_1m > 0 else 999
    ...
    if p30 < WATCH_VOLUME_PERCENT and (vol30 - prev30) < WHALE_VOLUME_DIFF_USDT:
        return None
    ...
    result = {
        ...
        "distance_1m": distance_1m
    }

    result["liquidity_score"] = liquidity_score(result)

    return result


def analyze_vip(symbol):
    base = analyze_liquidity(symbol)
    ...
    volume_score = base["p30"] * 0.55
    price_score = base["price_change_10m"] * 28
    liquidity_score_part = min(base["vol30"] / 10000, 65)
    entry_score = max(0, 45 - max(distance_4h_entry, 0) * 18)
    rr_score = min(levels_4h["rr"] * 12, 25)
    whale_bonus = 30 if base["vol_diff30"] >= WHALE_VOLUME_DIFF_USDT else 0

    early_bonus = 0
    if (
        base["p30"] >= EARLY_VOLUME_PERCENT
        and -0.2 <= base["price_change_10m"] <= 1.2
        and base["price_change_30m"] >= -0.7
        and distance_4h_entry <= 2.2
    ):
        early_bonus = 35

    score = volume_score + price_score + liquidity_score_part + entry_score + rr_score + whale_bonus + early_bonus
...
def get_liquidity_alert_type(a):
    if a["vol_diff30"] >= WHALE_VOLUME_DIFF_USDT * 3 or a["p30"] >= 180:
        return "liquidity_whale"

    if a["vol_diff30"] >= WHALE_VOLUME_DIFF_USDT or a["p30"] >= 100:
        return "liquidity_strong"

    if a["p30"] >= WATCH_VOLUME_PERCENT:
...
def create_performance_chart():
    try:
        ...
        print("matplotlib not available:", e, flush=True)
        return None
...
def check_report_command():
    ...
    try:
        ...
        for update in data.get("result", []):
            ...
            print("Command received:", text, flush=True)
...
/help - عرض الأوامر""",
        ...
        print("Report command error:", e, flush=True)
...
def liquidity_message(a, alert_type):
    strength = liquidity_strength(a)

    if alert_type == "liquidity_whale":
        title = "🚨🐋 <b>Liquidity Whale Alert</b>"
    elif alert_type == "liquidity_strong":
        title = "🔥📊 <b>Strong Liquidity Alert</b>"
    else:
        title = "📊 <b>مراقبة فوليوم v2</b>"

    return f"""{title}
<b>{a['pair']}</b> على Gate.io

🏷️ الحالة: <b>{strength}</b>
💰 السعر الحالي: <b>${a['price']:.6f}</b>

📊 فوليوم 30د: <b>{a['vol30']:,.0f} USDT</b>
🚀 ارتفاع الفوليوم 30د: <b>{a['p30']:.2f}%</b>
⬆️ زيادة الفوليوم: <b>{a['vol_diff30']:,.0f} USDT</b>

📊 فوليوم 60د: <b>{a['vol60']:,.0f} USDT</b>
📈 ارتفاع الفوليوم 60د: <b>{a['p60']:.2f}%</b>

🟢 تغير 10د: <b>{a['price_change_10m']:.2f}%</b>
📈 تغير 30د: <b>{a['price_change_30m']:.2f}%</b>

📌 دعم 30د: <b>${a['support_1m']:.6f}</b>
🧱 مقاومة 30د: <b>${a['resistance_1m']:.6f}</b>
📍 بعد السعر عن دعم 30د: <b>{a['distance_1m']:.2f}%</b>

🧠 Liquidity Score: <b>{a['liquidity_score']:.2f}</b>

⚠️ هذا تنبيه سيولة فقط، وليس صفقة VIP."""


def process_symbol(symbol):
    try:
        liquidity = analyze_liquidity(symbol)
        vip = analyze_vip(symbol)

        liquidity_result = None
        vip_result = None

        if liquidity:
            liq_type = get_liquidity_alert_type(liquidity)
        ...
        return liquidity_result, vip_result
...
print(f"Loaded Gate symbols: {len(symbols)}", flush=True)
print("VIP BOT + LIQUIDITY v2 + 4H ENTRY + REPORT RUNNING ✅", flush=True)
...
while True:
    try:
        ...
        for liquidity_result, vip_result in results:
            if liquidity_result:
                liquidity_candidates.append(liquidity_result)
            if vip_result:
                vip_candidates.append(vip_result)

        liquidity_candidates.sort(key=lambda x: x[0], reverse=True)
        vip_candidates.sort(key=lambda x: x[0], reverse=True)

        print(
            f"Liquidity candidates: {len(liquidity_candidates)} | VIP candidates: {len(vip_candidates)}",
            flush=True
        )

        liq_sent = 0

        for score, alert_type, a in liquidity_candidates:
            if liq_sent >= MAX_LIQUIDITY_ALERTS_PER_CYCLE:
                break

            symbol = a["symbol"]

            if not can_alert(symbol, alert_type, LIQUIDITY_COOLDOWN_SECONDS):
                continue

            print(
                f"Sending liquidity {symbol} | {alert_type} | "
                f"Liquidity Score: {score:.2f} | p30: {a['p30']:.2f}%",
                flush=True
            )

            send(liquidity_message(a, alert_type), symbol, CHAT_ID)
        ...
        for score, alert_type, a in vip_candidates:
            if vip_sent >= MAX_ALERTS_PER_CYCLE:
                break

            symbol = a["symbol"]

            if not can_alert(symbol, alert_type, ALERT_COOLDOWN_SECONDS):
                continue

            print(
                f"Sending VIP {symbol} | {alert_type} | "
        ...
        print(f"Cycle done | Liquidity Sent: {liq_sent} | VIP Sent: {vip_sent}\n", flush=True)
